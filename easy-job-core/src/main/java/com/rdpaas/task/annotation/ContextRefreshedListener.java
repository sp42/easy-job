...
public class ContextRefreshedListener implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        config.setSysStartTime(new Date()); // 初始化系统启动时间,用于解决系统重启后，还是按照之前时间执行任务
        taskRepository.reInitTasks(); // 重启重新初始化本节点的任务状态
        allTaskNames = taskRepository.listAllTaskNames(); // 查出数据库所有的任务名称
        ...
        if (event.getApplicationContext().getParent() == null) {
            ...
            Set<Map.Entry<String, Object>> entries = allBeans.entrySet();
            ...
            handleSheduledAnn(methodMap); // 处理Scheduled注解
            taskIdMap.clear(); // 由于taskIdMap只是启动spring完成后使用一次，这里可以直接清空
        }
    }
    ...
    private void handleSheduledAnn(Map<String, Method> methodMap, Method m) throws Exception {
        ...
        if (StringUtils.isEmpty(parent)) {
            ...
            if (parentTaskId == null) {
                ...
                parentTaskId = taskIdMap.get(parentMethodName); // 递归回来一定要更新一下这个父任务ID
            }
